    ...
    while (true) {
        ...
        for (uint8_t segIdx = 0; segIdx < 3; segIdx++) {
            ...
            for (long step = 0; step < totalSegSteps; step++) {
                ...
                // 1. ДИНАМИЧЕСКИЙ ПЕРЕСЧЕТ СКОРОСТИ ПО ФИЗИКЕ ШАГОВ
                // Вычисляем текущую частоту импульсов Мастер-Оси (в шагах/сек)
                float masterFrequency = currentVelocity * masterStepsPerMm;
                if (masterFrequency < 1.0f) masterFrequency = 1.0f;

                // Переводим частоту в количество пустых тактов нашей базовой 100 кГц сетки ЧПУ
                uint16_t delay100kHz = (uint16_t)(BASE_CNC_FREQ / (masterFrequency * pythonFeedrateOverride));
                if (delay100kHz < 1) delay100kHz = 1;

                // Точное время, которое станок потратит на этот шаг (в секундах)
                float stepDurationSec = (float)delay100kHz / BASE_CNC_FREQ; 

                // Наращиваем или гасим скорость вектора строго по физическому времени шага
                if (segIdx == 0) {
                    float deltaV = accel * stepDurationSec;
                    
                    // ЗАЩИТНЫЙ БАРЬЕР: Скорость за ОДИН шаг не имеет права вырасти 
                    // более чем на половину от разницы между стартом и маршем кадра!
                    ...
                    // Симметричный барьер для торможения
                ...
                // 2. ПОТАКТОВАЯ СБОРКА ЛЕНТЫ ВРЕМЕНИ ДЛЯ СЕТКИ 100 КГЦ
                // Размазываем этот шаг на один импульсный такт и серию пустых пауз
                for (uint16_t t = 0; t < delay100kHz; t++) {
                    
                    uint32_t nextHead = (bufferHead + 1) % STEP_BUFFER_SIZE;
                    while (nextHead == bufferTail) {
                        vTaskDelay(pdMS_TO_TICKS(1)); // Спим, если буфер полон
                    }

                    StepCmd cmd = {0, 0, 0, 0, 0, 0, 0};

                    // Физический импульс закладывается строго в первый квант времени
                    if (t == 0) {
                        // Чистый Брезенхем без флотов
                        errX -= segAbsX; if (errX < 0) { errX += totalSegSteps; cmd.stepX = 1; cmd.dirX = dirBitX; localStepsX += (dirBitX == 1) ? 1 : -1; }
                        errY -= segAbsY; if (errY < 0) { errY += totalSegSteps; cmd.stepY = 1; cmd.dirY = dirBitY; localStepsY += (dirBitY == 1) ? 1 : -1; }
                        errZ -= segAbsZ; if (errZ < 0) { errZ += totalSegSteps; cmd.stepZ = 1; cmd.dirZ = dirBitZ; localStepsZ += (dirBitZ == 1) ? 1 : -1; }
                    }
